...
const QuickEntryModal: React.FC<QuickEntryModalProps> = ({ 
  ...
  const handleSave = () => {
    ...
    addTask({
      ...
      myDay: stagedMeta.dueDate?.startsWith(new Date().toISOString().split('T')[0]) || false,
  ...
  return (
    <div className="modal active flex items-center justify-center p-4">
        ...
        <div className="flex flex-wrap gap-1.5 mt-4 min-h-[28px] items-center">
          ...
          {stagedMeta.dueDate && (
            <span className="tag-pill text-orange-600 dark:text-orange-400 flex items-center gap-1.5 font-bold">
              <i className="fas fa-calendar text-[9px]"></i> {new Date(stagedMeta.dueDate).toLocaleDateString()}
              <button onClick={() => setStagedMeta(s => ({ ...s, dueDate: null }))} className="hover:text-red-500 ml-0.5 text-[9px]">✕</button>
            </span>
          )}
            ...
            <div className="relative">
              ...
              {activePopover === 'schedule' && (
                <div className="absolute left-0 bottom-full mb-2 bg-[var(--bg-primary)] border border-[var(--border-color)] rounded-xl shadow-2xl p-3 z-50 min-w-[200px] flex flex-col gap-1">
                  ...
                  <button 
                    onClick={() => { setStagedMeta(s => ({ ...s, dueDate: new Date().toISOString().split('T')[0] + 'T12:00' })); setActivePopover(null); }}
                  ...
                  <button 
                    onClick={() => {
                      ...
                      setStagedMeta(s => ({ ...s, dueDate: tom.toISOString().split('T')[0] + 'T12:00' }));
                      setActivePopover(null);
                    }}
                    className="text-left px-2.5 py-1.5 text-xs font-semibold hover:bg-[var(--bg-secondary)] rounded-md flex items-center gap-2"
                  >
                  ...
                  <button 
                    onClick={() => {
                      ...
                      setStagedMeta(s => ({ ...s, dueDate: nextW.toISOString().split('T')[0] + 'T12:00' }));
                      setActivePopover(null);
                    }}
                    className="text-left px-2.5 py-1.5 text-xs font-semibold hover:bg-[var(--bg-secondary)] rounded-md flex items-center gap-2"
                  >
      ...
      <input 
        ...
        className="hidden" 
        onChange={(e) => {
          setStagedMeta(s => ({ ...s, dueDate: e.target.value }));
          inputRef.current?.focus();
        }}
      />
    </div>
